...
class BotPathSender {
  ...
  constructor(options: BotPath) {
    const { wordlistDir } = ConfigService.getInstance().getConfig();
    const wordlistFile = fs.readFileSync(wordlistDir, 'utf8').trim();
    this.wordlist = wordlistFile.split('\n');
    this.options = options;
  }

  private getOptFromPacketAndNewPath(
  ...
  prepare(packet: PacketRequest, currentPath: string): AxiosRequestConfig[] {
    const opts = this.wordlist.map((w) =>
      this.getOptFromPacketAndNewPath(packet, currentPath, w),
    );
    opts.push(
      this.getOptFromPacketAndNewPath(packet, currentPath, randomHex(32)),
    );
  ...
  async sendAll(
    requestsOptions: AxiosRequestConfig[],
  ): Promise<AxiosResponse<any>[]> {
    let cnt = 0;
    const resps = await Bluebird.map(
      requestsOptions,
      async (opts) => {
        ...
        if (cnt % 30 === 0) {
          console.log(
            `Done ${cnt}/${requestsOptions.length}: ${opts.baseURL}${opts.url}`,
          );
        }
        try {
